...
The final result of \vref{eq:twoDim} with the boundaries \vref{eq:twoDimBoundaries} is 
...
\mu L^2 &= \qty(k\pi)^2 \nonumber \\
 \mu  &=\qty(\frac{k\pi}{L})^2 \label{eq:twoDimMu}
\end{align}
...
\begin{align*}
	X(x) = c_1\cos(\sqrt{\mu}x) + c_2\sin(\sqrt{\mu}y)
\end{align*}
\end{document}
